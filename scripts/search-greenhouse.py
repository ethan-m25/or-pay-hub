#!/usr/bin/env python3
"""
or-pay-hub/scripts/search-greenhouse.py
Greenhouse job board scraper — Oregon edition.
Oregon ORS 652.220 (HB 2016, eff. Jan 1, 2023) — employers with 10+ employees must include pay range in all job postings
Run: python3 ~/or-pay-hub/scripts/search-greenhouse.py
"""
...
LOG_FILE  = os.path.expanduser("~/or-pay-hub/scripts/greenhouse.log")
LOCK_FILE = os.path.expanduser("~/or-pay-hub/scripts/.greenhouse.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"

log = make_logger(LOG_FILE)
fetcher = Fetcher()

SEED_SLUGS = [('nike', None), ('adidas', None), ('vacasa', None), ('puppet', None), ('gremlin', None), ('smarsh', None), ('galois', None), ('oha', 'Oregon Health Authority'), ('cambia', None), ('onpoint', 'OnPoint Community Credit Union'), ('zapproved', None), ('webmd', None), ('daimlertruck', 'Daimler Truck'), ('pcna', 'Precision Castparts'), ('tripwire', None)]
    ...
    for pat in SALARY_PATTERNS:
        ...
        if m:
            try:
                ...
                if val_min < 1000: val_min *= 1000
                if val_max < 1000: val_max *= 1000
                if 30000 <= val_min < val_max <= 1500000:
...
def is_region_job(title: str, location: str, content: str) -> bool:
...
def fetch_company_jobs(slug: str, company_name_override=None):
    ...
    for j in jobs_raw:
        ...
        if not is_region_job(title, location, content_text):
            continue
...
def main():
    ...
    log("=== OR Greenhouse scraper started ===")
    ...
    log(f"=== Done. {new_count} new OR jobs written to {OUTPUT_FILE} ===")
