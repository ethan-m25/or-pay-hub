#!/usr/bin/env bash
# or-pay-hub/scripts/nightly-pipeline.sh
...
SCRIPTS_DIR="$HOME/or-pay-hub/scripts"
...
log "=== OR nightly pipeline started ==="
...
python3 "$HOME/shared-scripts/run_locked.py" /tmp/ollama-model.lock \
    python3 "$HOME/shared-scripts/hub_classify_new_jobs.py" --hub "or" >> "$LOG_FILE" 2>&1 || true
