#!/usr/bin/env python3
"""
or-pay-hub/scripts/search-workday.py
Workday API scraper — Oregon edition.

Oregon Pay Equity Act HB 2005 (effective Jan 1, 2023)

Strategy:
  1. Tenant discovery — Exa search finds *.myworkdayjobs.com URLs → extract Oregon employers
  2. CXS JSON API   — paginate all jobs per employer, filter Oregon (fast, no JS)
  3. HTML job page  — salary in raw HTML; pure regex, no LLM

Run: python3 ~/or-pay-hub/scripts/search-workday.py
"""
...
LOG_FILE      = os.path.expanduser("~/or-pay-hub/scripts/workday.log")
LOCK_FILE     = os.path.expanduser("~/or-pay-hub/scripts/.workday.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"
LARGE_TENANT_THRESHOLD = 500
REGION_SEARCH_TEXT = "Oregon"
...
# ── Seed tenants ─────────────────────────────────────────────────────────────
SEED_TENANTS = [
    ("nike.wd1.myworkdayjobs.com", "nike", "External", "Nike"),
...
DISCOVERY_QUERIES = [
    'site:myworkdayjobs.com "Oregon" salary 2026',
    'site:myworkdayjobs.com "Portland" salary "$" 2026',
    'site:myworkdayjobs.com "Beaverton" OR "Hillsboro" OR "Eugene" salary 2026',
    'site:myworkdayjobs.com Oregon tech OR semiconductor OR healthcare salary 2026',
    'site:myworkdayjobs.com "Salem" OR "Bend" Oregon salary 2026',
]
...
def is_oregon(locations_text, external_path=""):
    """Return True only if the job is plausibly located in Oregon.
...
def main():
    ...
    for host, company_id, tenant, company_name in all_tenants:
        ...
        while offset // limit < max_pages:
            ...
            for p in postings:
                if is_oregon(p.get("locationsText", ""), p.get("externalPath", "")):
                    or_jobs.append(p)
            offset += limit
        ...
        log(f"  Oregon jobs: {len(or_jobs)}")

        # Fetch HTML for each BC job and extract salary
        for i, posting in enumerate(or_jobs, 1):
    ...
    for index, (url, meta) in enumerate(fallback_candidates, 1):
        ...
        if not is_oregon(location, external_path):
            continue
